...
def get_pie_chart(entered_site):
    ...
    if entered_site == 'ALL':
        ...
        filtered_df['name'] = ['Success', 'Failure']
        fig = px.pie(filtered_df, values='class',
        names='name', title='Total Success Launches for Site ' + entered_site)
        return fig
...
def get_scatter(site, load):
    if site == "ALL":
        filtered_df = spacex_df
        filtered_df = filtered_df[(filtered_df['Payload Mass (kg)'] >= load[0]) & (filtered_df['Payload Mass (kg)'] <= load[1])]
